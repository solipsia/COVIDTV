...
void settings() {
    ...
    sql = new MySQL( this, "localhost", "COVID", "root", "313920Raspberry" );
 //sql = new MySQL( this, "localhost", "COVID", "root", "313920" );
  ...
  countries.add(new Country("World","ZA",worldlatestconfirmed,worldyesterdayconfirmed,worldlatestdeaths,worldyesterdaydeaths,countries.get(0).latestdate,countries.get(0).secondlatestdate));
...
void draw() {
  ...
  fill(255,0,100);noStroke();textAlign(LEFT);textSize(13);text((int)frameRate,0,height-4);
  fill(255,0,255);noStroke();textAlign(LEFT);textSize(13);text((new SimpleDateFormat("dd/MM/yyyy HH:mm:ss")).format((new Date()))+" UTC",1180,1070);
...
void reportPercentageofWorld() {
    ...
    noStroke();fill(255,0,255,255);textSize(26);text("Total World Population: "+prettyLong(currentWorldPopulation()),110,620);    
}
...
class Country {
  ...
  Country (String name_,String iso_, int latestconfirmed_, int yesterdayconfirmed_, int latestdeaths_, int yesterdaydeaths_, Date latestdate_, Date secondlatestdate_) {
    ...
    //}
    //flag = loadImage("flagicons/za.png");
  }
